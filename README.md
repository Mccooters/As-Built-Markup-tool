...
Home's **Projects** card is one list of every job this device or the team cloud knows about, grouped by status: **In progress**, **DLP** (defects liability period) and **Completed / archived** (collapsed, with a count — tap the heading to expand; the choice sticks per device). Each row shows the site or client, the AroFlo project number, who saved last and whether the job is **on this device** (opens offline) or only in the **team cloud** (tap to download it). Change a job's status from the dropdown on its row or in Project details: the open drawing changes in place, a stored drawing is patched on the device, and a team project is updated in the registry **without touching its drawing or bumping its version**, so nobody gets a conflict warning over a list change. The registry's status is the team's shared truth — it wins when a project is downloaded, and reloading the list (⟳, or coming back to Home) adopts a status a teammate changed.
