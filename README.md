...
6. **Draw area zones** — drag a box over an area of the overall drawing and link it to the AroFlo tasks for that area (the picker lists the project's tasks; extra job numbers can be typed). Tapping a zone with Select pops up its jobs, their status, and the **materials already used** on each — the drawing becomes a live map of the project. Zones never gray out in Day mode, print in exports as an area key, and their links save with the project.
...
- **Catalogue-layout stocktakes** — the **Catalogue** toggle in stocktake mode lays the count out like the press-fitting catalogue: one section per fitting family (couplings, slip couplings, elbows 90°/45°, tees, reducing tees, end caps…) with a cell per size showing AroFlo's figure and the count box — matching how the shelves are actually organised. Counts carry over when switching between catalogue and list, search narrows to families, and the preference sticks.
